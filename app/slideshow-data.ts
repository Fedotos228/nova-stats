...
export async function buildSlides(): Promise<Slide[]> {
  ...
  // The refresh (local task or GitHub Actions fallback, see
  // scripts/windows/install-scheduled-tasks.ps1 and
  // .github/workflows/refresh-weather.yml) runs in the afternoon, not at UTC midnight, so
  // the buster must roll over then too — otherwise it stays unchanged across the actual
  // image refresh and only flips 12h later, at midnight.
