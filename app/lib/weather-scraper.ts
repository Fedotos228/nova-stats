...
export async function refreshWeatherImages() {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "weather-"))
  try {
    const manifest = await Promise.all(
      urls.map(async (url, i) => {
        ...
        const input = path.join(tmpDir, `raw-${name}`)
        fs.writeFileSync(input, buffer)
        return { input, output: path.join(tmpDir, `up-${name}`), name }
      }),
    )

    await upscaleImages(manifest.map(({ input, output }) => ({ input, output })))

    const freshPathnames = await Promise.all(
      manifest.map(async ({ output, name }) => {
        const buffer = fs.readFileSync(output)
        const blob = await put(`${BLOB_PREFIX}${name}`, buffer, {
          access: "public",
